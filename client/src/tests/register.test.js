...
describe("RegisterPage", () => {
    describe("Valid inputs", () => {
        it("calls the onSubmit function", async () => {
            ...
            await act(async () => {
                fireEvent.change(getByLableText("Email *"), {
                    target: { value: "eamil@test.com" },
                });
                fireEvent.change(getByLableText("Felhasználónév *"), {
                    target: { value: "TesztElek" },
                });
                fireEvent.change(getByLableText("Jelszó *"), {
    ...
    describe("Invalid username", () => {
        it("renders the username validation error", async () => {
            ...
            expect(container.innerHTML).toMatch(
                "A felhasználónévnek többnek kell lennie mint 5 karakter"
            );
        });
    });
    ...
    describe("Invalid password", () => {
        it("renders the password validation error", async () => {
            ...
            expect(container.innerHTML).toMatch(
                "A jelszónak többnek kell lennie mint 5 karakter"
            );
        });
    });
});
