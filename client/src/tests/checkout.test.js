...
describe("CheckOutPage", () => {
    describe("Valid inputs", () => {
        it("calls the onSubmit function", async () => {
            ...
            render(
                <UserContext.Provider value={providerTestUser}>
                    <ShoppingCartContext.Provider value={providerTestCart}>
                        <AppRouter>
                            <CheckOutPage onSubmit={mockOnSubmit} />
                        </AppRouter>
            ...
            await act(async () => {
                fireEvent.change(getByLabelText("Vezetéknév *"), {
                    target: { value: "Kiss" },
                });
                fireEvent.change(getByLabelText("Kersztnév *"), {
                    target: { value: "Attila" },
                });
                fireEvent.change(getByLabelText("Lakcím *"), {
                    target: { value: "teszt u. 7." },
                });
                fireEvent.change(getByLabelText("Kártya szám *"), {
                    target: { value: "1457-1458-2586-5852" },
                });
            });
            await act(async () => {
        ...
        it("Ki írja ha hibás a lakcím", async () => {
            ...
            await act(async () => {
                ...
                fireEvent.change(addressInput, {
                    target: { value: "Níregháza teszt u, 7" },
                });
                fireEvent.blur(addressInput);
            });

            expect(container.innerHTML).toMatch(
                "A lakcímnek érvényesnek kell lennie."
            );
        });
    });
        ...
        it("Ki írja ha hibás a kártya szám", async () => {
            ...
            await act(async () => {
                ...
                fireEvent.change(cardnumberInput, {
                    target: { value: "145-18573-748952-885496" },
                });
                fireEvent.blur(cardnumberInput);
            });

            expect(container.innerHTML).toMatch(
                "A kártya számnak érvényesnek kell lennie"
            );
        });
    });
});
