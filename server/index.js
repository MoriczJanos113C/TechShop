...
app.post("/products", upload.single("file"), (req, res) => {
    ...
    db.query(
        `INSERT INTO product (cost, category, name, description, image)  VALUES (?, ?, ?, ?, ?)`,
        [cost, category, name, description, image],
        (err, result) => {
            if (err) throw err;
            if (result) {
                res.send(result);
...
app.put("/products/:id", upload.single("file"), async (req, res) => {
    ...
    db.query(
        `UPDATE product SET cost = ?, category = ?, name = ?, description = ?, image = ? WHERE id = ${req.params.id}`,
        [cost, category, name, description, image],
        (err, result) => {
            if (err) throw err;
    ...
    db.query(
        ...
        (err, result) => {
            if (result) {
                ...
                res.send({
                    message: "Not deleted any product",
                });
            }
        }
    );
...
app.post("/review", async (req, res) => {
    ...
    db.query(
        "INSERT INTO reviews (user_id, product_id, description, rating, username) VALUES (?, ?, ?, ?, ?)",
...
app.post("/entries", (req, res) => {
    ...
    db.query(
        `INSERT INTO entries (user_id, username, title, description)  VALUES (?, ?, ?, ?)`,
...
app.put("/entries/:id", async (req, res) => {
    ...
    db.query(
        `UPDATE entries SET title = ?, description = ? WHERE id = ${req.params.id}`,
        [title, description],
        (err, result) => {
            if (err) throw err;
...
app.delete("/deleteEntries/:id", (req, res) => {
    db.query(
        `DELETE FROM entries WHERE id = ${req.params.id}`,
        (err, result) => {
            if (result) {
                res.send({
...
app.post("/register", async (req, res) => {
    ...
    db.query(
        ...
        (err, result) => {
            ...
            if (result.length === 0) {
                db.query(
                    "INSERT INTO user (username, email, password, role) VALUES (?, ?, ?, 'normal')",
...
app.post("/checkout", async (req, res) => {
    ...
    db.query(
        "INSERT INTO orders (user_id, username, email, contactInfo, items, itemName, totalCost) VALUES (?, ?, ?, ?, ?, ?, ?)",
        [
            user_id,
            username,
            email,
...
app.put("/users/:id", async (req, res) => {
    ...
    db.query(
        `UPDATE user SET username = ?, password = ?,email = ?, role = ?  WHERE id = ${req.params.id}`,
        [username, hashedPass, email, role],
        (err, result) => {
            if (err) throw err;
            if (result) {
    ...
    db.query(
        ...
        (err, result) => {
            if (result) {
                ...
                res.send({
                    message: "Not found any order for this user",
                });
            }
        }
    );
